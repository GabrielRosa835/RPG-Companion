...
public readonly record struct Maybe<T>
{
    ...
    public static Maybe<T> Perhaps(T? value) => value is not null ? Some(value) : None();

    public static implicit operator Maybe<T>(T? value) => Perhaps(value);
    public static implicit operator Maybe<T>(None _) => None();
    public static implicit operator bool(Maybe<T> maybe) => maybe.IsPresent;

    public TResult Either<TResult>(Func<T, TResult> onSome, Func<TResult> onNone)
